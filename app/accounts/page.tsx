...
export default function AccountsPage() {
      ...
      try {
        ...
        if (data && data.length > 0) {
          setAccounts(data.map(r => ({
            ...
            monthlySpend: r.monthly_spend, status: r.status as any,
            campaigns: r.campaigns, roas: parseFloat(r.roas),
          })))
        } else {
          setAccounts(CLIENT_ACCOUNTS)
        }
      } catch {
        setAccounts(CLIENT_ACCOUNTS)
      } finally {
  ...
  const avgRoas = accounts.length > 0 ? accounts.reduce((s, a) => s + a.roas, 0) / accounts.length : 0
  ...
  const handleCreate = async () => {
    ...
    try {
      const { data, error: dbError } = await supabase
        .from('client_accounts')
        .insert({ name: newName.trim(), initials, platforms: newPlatforms, monthly_spend: spend, status: 'Active', campaigns: 0, roas: 0 })
        .select().single()
      if (dbError) throw dbError
      setAccounts(prev => [...prev, {
        id: data.id, name: data.name, initials: data.initials,
        platforms: data.platforms as Platform[],
        monthlySpend: data.monthly_spend, status: 'Active', campaigns: 0, roas: 0,
      }])
    } catch {
      setAccounts(prev => [...prev, {
        id: String(Date.now()), name: newName.trim(), initials,
        platforms: newPlatforms, monthlySpend: spend, status: 'Active', campaigns: 0, roas: 0,
      }])
    } finally {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-4 gap-3.5 mb-7">
        <KpiCard label="Total Accounts" value={String(accounts.length)} change={1} changeLabel="persisted in database" icon={<Users size={14} />} />
        <KpiCard label="Active Platforms" value={String(activePlatforms)} change={0} changeLabel="no change" icon={<Layers size={14} />} />
        <KpiCard label="Avg ROAS" value={avgRoas > 0 ? `${avgRoas.toFixed(1)}x` : '—'} change={0.3} changeLabel="0.3x from last month" icon={<Star size={14} />} highlight />
      ...
      <div className="flex items-center justify-between mb-3.5">
        ...
        <div className="flex gap-2">
          ...
          <select className="text-[12px] px-2.5 py-1.5 rounded-[7px] border outline-none" style={inputStyle}>
            <option>Sort: ROAS (high–low)</option><option>Sort: Spend (high–low)</option><option>Sort: Name (A–Z)</option>
          </select>
        </div>
      </div>
      <div className="rounded-xl border overflow-hidden" style={{ borderColor: 'var(--border)', background: 'var(--bg2)' }}>
        {loading ? (
          <div className="flex items-center justify-center gap-3 py-16" style={{ color: 'var(--text2)' }}>
            <Spinner /><span className="text-[13px]">Loading accounts...</span>
          </div>
        ) : accounts.length === 0 ? (
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center"
          ...
          <div className="rounded-xl border p-6 w-full max-w-md"
            ...
            <div className="text-[12px] mb-5" style={{ color: 'var(--text2)' }}>Saved permanently to your database.</div>
            <div className="mb-4">
